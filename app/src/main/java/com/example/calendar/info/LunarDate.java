...
public class LunarDate implements Serializable {
    ...
    public String monthDisplay; //月显示
    public String dayDisplay; //日显示
    public String lunarDisplay; //日历显示

    private static final String[] MonthNames = {"正","二","三","四","五","六","七","八","九","十","冬","腊"};
    private static final String[] DayNumbers = {"一","二","三","四","五","六","七","八","九"};
    private static final String[] HeavenlyStems = {"癸","甲","乙","丙","丁","戊","己","庚","辛","壬"};
    private static final String[] EarthlyBranches = {"亥","子","丑","寅","卯","辰","巳","午","未","申","酉","戌"};
    private static final String[] Zodiacs = {"猪","鼠","牛","虎","兔","龙","蛇","马","羊","猴","鸡","狗"};
    private static final String[] TenDays = {"初","十","廿"};
    ...
    public String getLunarDisplay() { return lunarDisplay; }

    public String getLunarInformation() { return "农历" + monthDisplay + dayDisplay; }
    ...
    private static String getChineseZodiac(int lunarYear) {
        return Zodiacs[(lunarYear - 3) % 12];
    }

    private static String getHSaEB(int lunarYear) {
        return HeavenlyStems[(lunarYear - 3) % 10] + EarthlyBranches[(lunarYear - 3) % 12];
    }

    public static String getHEZ(int lunarYear) {
        return getHSaEB(lunarYear) + getChineseZodiac(lunarYear) + "年";
    }

    public static String getMonthDisplay(int lunarMonth, boolean isLeap) {
        return isLeap ? "闰" : "" + MonthNames[lunarMonth - 1] + "月";
    }

    public static String getDayDisplay(int lunarDay) {
        if(lunarDay == 10) {return "初十";}
        else if(lunarDay == 20) {return "二十";}
        else if(lunarDay == 30) {return "三十";}
        else {
            int n = lunarDay % 10 == 0 ? 9 : (lunarDay % 10 - 1);
            return TenDays[lunarDay / 10] + DayNumbers[n]; }
    }

    public static String getLunarDisplay(String monthDisplay, String dayDisplay) {
       if(dayDisplay.equals("初一")) return monthDisplay;
       else return dayDisplay;
    }

    public static LunarDate getLunar(LocalDate localDate) {
        ...
        for (int i = 0; i < 13; i++) {
            ...
            if (offset > dm) {lunarMonth++; offset -= dm;}
            else break;}
        int lunarDay = (int) (offset);
        ...
        lunar.monthDisplay = getMonthDisplay(lunar.lunarMonth, lunar.isLeap);
        lunar.dayDisplay = getDayDisplay(lunar.lunarDay);
        lunar.lunarDisplay = getLunarDisplay(lunar.monthDisplay, lunar.dayDisplay);
        return lunar;
    }
